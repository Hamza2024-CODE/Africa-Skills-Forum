<div class="h-screen w-screen flex flex-col bg-[#F8FAFC] relative overflow-hidden text-start font-sans"
    ...
    <div class="relative z-20 flex items-center justify-between px-8 py-4 bg-white border-b border-slate-200 shadow-sm shrink-0">
        <div class="flex items-center gap-4">
            <img src="/AFRICA.png" alt="{{ platform()->name() }}" class="h-11 w-auto object-contain">
            <div>
                <h1 class="text-lg font-black text-[#0B2A6F] tracking-wide leading-none">{{ platform()->name() }}</h1>
                <p class="text-[11px] font-bold text-[#35A536] tracking-wider uppercase mt-1">منصة البث المباشر والشاشات التلفزيونية الرسمية</p>
            </div>
        </div>
        ...
        <div class="flex items-center gap-4">
            ...
            <div class="text-base font-mono font-black text-[#0B2A6F] bg-slate-100 px-4 py-2 rounded-2xl border border-slate-200 shadow-inner" 
                 x-data="{ t: '' }" 
        ...
        @elseif($slides->isNotEmpty())
            ...
            @foreach($slides as $idx => $slide)
                <div x-show="currentSlide === {{ $idx }}" 
                    ...
                    <span class="text-xs font-black text-[#0B2A6F] uppercase tracking-widest px-4 py-1.5 rounded-full bg-blue-50 border border-blue-200 inline-block shadow-xs">
                        ✦ {{ $slide->slide_type }}
                    </span>
                    
                    <h2 class="text-4xl sm:text-6xl font-black text-[#0B2A6F] leading-tight drop-shadow-xs max-w-4xl">
                        {{ $slide->title_ar }}
                    </h2>
            ...
            <div class="absolute bottom-8 flex items-center gap-3 z-30">
                @foreach($slides as $idx => $slide)
                    <button @click="currentSlide = {{ $idx }}"
                        ...
                        :class="currentSlide === {{ $idx }} ? 'bg-[#0B2A6F] w-10' : 'bg-slate-300 w-3 hover:bg-blue-400'">
                    </button>
                @endforeach
            </div>
            ...
            <div class="text-center space-y-8 max-w-4xl mx-auto">
                <div class="relative inline-block">
                    <div class="w-32 h-32 rounded-3xl bg-blue-50 border-2 border-blue-200 flex items-center justify-center mx-auto shadow-xl transform hover:rotate-3 transition-transform">
                        <img src="/AFRICA.png" alt="{{ platform()->name() }}" class="h-20 w-auto object-contain">
                    </div>
                ...
                <div class="p-6 rounded-3xl bg-slate-50 border border-slate-200/80 shadow-md max-w-xl mx-auto space-y-2">
                    <p class="text-sm font-black text-[#06205C] flex items-center justify-center gap-2">
                        ...
                        <span>مركز المؤتمرات محمد بن أحمد (CCO) — ولاية وهران</span>
                    </p>
                    <p class="text-xs text-slate-500 font-bold">
                        تغطية مباشرة وشاملة للمؤتمرات ورشات المهارات المسجلة.
                    </p>
                </div>
            </div>
        @endif
