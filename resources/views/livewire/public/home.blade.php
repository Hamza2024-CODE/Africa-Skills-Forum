<div class="space-y-12 pb-16" x-data="{ showVideoModal: false }">
    ...
    <!-- 2. High-Tech Professional Chronometer Clock with Automatic Clock-Tick Audio -->
    <section class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 -mt-20 relative z-30" 
             x-data="{
                 days: {{ $eventCountdown['days'] ?? '29' }},
                 hours: {{ $eventCountdown['hours'] ?? '23' }},
                 minutes: {{ $eventCountdown['minutes'] ?? '36' }},
                 seconds: {{ $eventCountdown['seconds'] ?? '45' }},
                 soundMuted: false,
                 audioCtx: null,
                 timer: null,
                 init() {
                     // Auto setup audio context on user interaction or first tick
                     const setupAudio = () => {
                         if (!this.audioCtx) {
