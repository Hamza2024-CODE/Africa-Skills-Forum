<div class="min-h-screen py-10 px-4 flex flex-col items-center justify-center bg-white font-sans print:bg-white print:py-0 print:px-0">
    ...
    <div class="w-full max-w-xl mb-8 flex flex-wrap items-center justify-between gap-3 text-slate-900 print:hidden bg-white border border-slate-200 p-4 rounded-2xl shadow-xl">
        <a href="{{ $badgeBackRoute }}" class="px-4 py-2.5 rounded-xl bg-slate-100 hover:bg-slate-200 border border-slate-200 text-xs font-bold text-slate-800 transition flex items-center gap-1.5 shadow-xs">
            ...
            <span>{{ app()->getLocale() === 'pt' ? (__('الرجوع') !== 'الرجوع' ? __('الرجوع') : 'Back') : (polyTrans('الرجوع', 'Retour', 'Back')) }}</span>
        </a>
        ...
        <div class="flex items-center gap-2">
            ...
            <button type="button" onclick="downloadBadgeAsImage()" class="px-4 py-2.5 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white font-black text-xs shadow-md transition flex items-center gap-2 border border-emerald-500">
                ...
                <span>{{ app()->getLocale() === 'pt' ? (__('تحميل الشارة كصورة HD 🖼️') !== 'تحميل الشارة كصورة HD 🖼️' ? __('تحميل الشارة كصورة HD 🖼️') : 'Download Image (HD)') : (polyTrans('تحميل الشارة كصورة HD 🖼️', 'Télécharger Image (PNG)', 'Download Image (HD)')) }}</span>
            </button>
            ...
            <button onclick="window.print()" class="px-5 py-2.5 rounded-xl bg-gradient-to-r from-[#06205C] to-[#0A3580] hover:from-[#041640] hover:to-[#06205C] text-amber-300 font-black text-xs shadow-lg shadow-[#06205C]/20 transition flex items-center gap-2 border border-amber-400/40">
                ...
                <span>{{ app()->getLocale() === 'pt' ? (__('طباعة الشارة الرسمية 🖨️') !== 'طباعة الشارة الرسمية 🖨️' ? __('طباعة الشارة الرسمية 🖨️') : 'Print PVC Badge') : (polyTrans('طباعة الشارة الرسمية 🖨️', 'Imprimer PVC', 'Print PVC Badge')) }}</span>
            </button>
        </div>
    </div>

    @php
        $rawJobOrPos = mb_strtoupper(($registration?->job_title ?? '') . ' ' . ($user?->position ?? '') . ' ' . ($roleTitle ?? ''));
        
        $theme = match(true) {
            str_contains($rawJobOrPos, 'VVIP') || str_contains($rawJobOrPos, 'سامية') => [
                'card_bg' => 'linear-gradient(145deg, #78350F 0%, #451A03 45%, #1C0601 100%)',
                'lanyard_filter' => 'hue-rotate(45deg) saturate(3) brightness(1.2)',
                'edge_color' => '#290D01',
                'rim_grad' => '#ffffff',
                'text_accent' => '#FDE68A',
                'divider_glow' => '#F59E0B',
                'badge' => 'شخصية سامية جداً — VVIP GUEST',
                'badge_bar_bg' => 'linear-gradient(90deg, rgba(245,158,11,0.4) 0%, rgba(217,119,6,0.7) 50%, rgba(245,158,11,0.4) 100%)',
                'bar_border' => '#FDE68A',
            ],
            str_contains($rawJobOrPos, 'VIP') || str_contains($rawJobOrPos, 'شرف') => [
                'card_bg' => 'linear-gradient(145deg, #831843 0%, #500724 50%, #1F010D 100%)',
                'lanyard_filter' => 'hue-rotate(300deg) saturate(2.5) brightness(1.0)',
                'edge_color' => '#1F010D',
                'rim_grad' => '#ffffff',
                'text_accent' => '#FBCFE8',
                'divider_glow' => '#F472B6',
                'badge' => 'ضيف شرف — VIP GUEST',
                'badge_bar_bg' => 'linear-gradient(90deg, rgba(236,72,153,0.3) 0%, rgba(244,114,182,0.5) 50%, rgba(236,72,153,0.3) 100%)',
                'bar_border' => '#FBCFE8',
            ],
            str_contains($rawJobOrPos, 'DIPLOMAT') || str_contains($rawJobOrPos, 'دبلوماسي') || str_contains($rawJobOrPos, 'سفار') => [
                ...
                'text_accent' => '#DDD6FE',
                'divider_glow' => '#C084FC',
                'badge' => 'دبلوماسي / مبعوث سفارة — DIPLOMAT',
                'badge_bar_bg' => 'linear-gradient(90deg, rgba(168,85,247,0.3) 0%, rgba(192,132,252,0.4) 50%, rgba(168,85,247,0.3) 100%)',
                'bar_border' => '#DDD6FE',
            ],
            str_contains($rawJobOrPos, 'DELEGATION HEAD') || str_contains($rawJobOrPos, 'DELEGATION_HEAD') || str_contains($rawJobOrPos, 'رئيس الوفد') || str_contains($rawJobOrPos, 'مسؤول الوفد') => [
                ...
                'text_accent' => '#6EE7B7',
                'divider_glow' => '#34D399',
                'badge' => 'رئيس الوفد الوطني — DELEGATION HEAD',
                'badge_bar_bg' => 'linear-gradient(90deg, rgba(16,185,129,0.3) 0%, rgba(52,211,153,0.4) 50%, rgba(16,185,129,0.3) 100%)',
                'bar_border' => '#34D399',
            ],
            str_contains($rawJobOrPos, 'COORDINATOR') || str_contains($rawJobOrPos, 'مؤطر') || str_contains($rawJobOrPos, 'مرافق') => [
                'card_bg' => 'linear-gradient(145deg, #0F766E 0%, #115E59 50%, #042F2E 100%)',
                'lanyard_filter' => 'hue-rotate(150deg) saturate(2) brightness(0.9)',
                'edge_color' => '#031E1D',
                'rim_grad' => '#ffffff',
                'text_accent' => '#99F6E4',
                'divider_glow' => '#2DD4BF',
                'badge' => 'مؤطر ومرافق تنفيذي — DELEGATION COORDINATOR',
                'badge_bar_bg' => 'linear-gradient(90deg, rgba(20,184,166,0.3) 0%, rgba(45,212,191,0.4) 50%, rgba(20,184,166,0.3) 100%)',
                'bar_border' => '#99F6E4',
            ],
            str_contains($rawJobOrPos, 'OFFICIAL_MEMBER') || str_contains($rawJobOrPos, 'عضو') || str_contains($rawJobOrPos, 'MEMBRE') => [
                'card_bg' => 'linear-gradient(145deg, #0284C7 0%, #0369A1 50%, #022B45 100%)',
                'lanyard_filter' => 'hue-rotate(190deg) saturate(2) brightness(1.0)',
                'edge_color' => '#011C2E',
                'rim_grad' => '#ffffff',
                'text_accent' => '#BAE6FD',
                'divider_glow' => '#38BDF8',
                'badge' => 'عضو رسمي في الوفد — OFFICIAL MEMBER',
                'badge_bar_bg' => 'linear-gradient(90deg, rgba(2,132,199,0.3) 0%, rgba(56,189,248,0.4) 50%, rgba(2,132,199,0.3) 100%)',
                'bar_border' => '#BAE6FD',
            ],
            str_contains($rawJobOrPos, 'SPEAKER') || str_contains($rawJobOrPos, 'محاضر') || str_contains($rawJobOrPos, 'متحدث') => [
                'card_bg' => 'linear-gradient(145deg, #047857 0%, #065F46 50%, #022C22 100%)',
                'lanyard_filter' => 'hue-rotate(120deg) saturate(2) brightness(0.9)',
                'edge_color' => '#011A13',
                'rim_grad' => '#ffffff',
                'text_accent' => '#A7F3D0',
                'divider_glow' => '#34D399',
            ...
            str_contains($rawJobOrPos, 'EXPERT') || str_contains($rawJobOrPos, 'خبير') || str_contains($rawJobOrPos, 'محكّم') => [
            ...
            str_contains($rawJobOrPos, 'MEDIA') || str_contains($rawJobOrPos, 'صحف') || str_contains($rawJobOrPos, 'إعلام') => [
                ...
                'text_accent' => '#FDE68A',
                'divider_glow' => '#F59E0B',
                'badge' => 'صحافة وإعلام — MEDIA & PRESS',
