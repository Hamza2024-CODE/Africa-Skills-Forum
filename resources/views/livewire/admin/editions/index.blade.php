...
<div class="space-y-5 pb-8" dir="{{ $locale === 'ar' ? 'rtl' : 'ltr' }}">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            ...
            <div class="flex items-center gap-3">
                ...
                <div>
                    <h1 class="text-2xl font-black text-slate-900 dark:text-slate-100">
                        {{ $t('إدارة الطبعات والدورات الوطنية', 'Gestion des Éditions Nationales', 'Editions & Olympic Sessions Management') }}
                    </h1>
                    <p class="text-sm font-medium text-slate-500 dark:text-slate-400">
                        {{ $t('إجمالي الطبعات المسجلة: ', 'Total Éditions: ', 'Total Editions: ') }}<span class="text-blue-600 dark:text-blue-400 font-bold">{{ $totalEditions }}</span>
                    </p>
                </div>
            </div>
        </div>
        <button wire:click="openCreate"
            ...
            <span>{{ $t('إضافة طبعة', 'Ajouter Édition', 'Add Edition') }}</span>
        </button>
    </div>
