<div class="space-y-8" x-data="{ showBadgeModal: false }">
    ...
    <div class="relative overflow-hidden rounded-3xl bg-gradient-to-r from-[#020A24] via-[#06205C] to-[#0052CC] text-white p-6 sm:p-10 shadow-2xl border border-white/10">
        ...
        <div class="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
                    ...
                    <div class="w-20 h-20 sm:w-24 sm:h-24 rounded-2xl bg-gradient-to-tr from-amber-400 via-brand-sky to-white p-1 shadow-xl overflow-hidden">
                        @php
                            ...
                            if ($profile && $profile->photo_path) {
                                $photoUrl = asset('storage/' . ltrim($profile->photo_path, '/'));
                            } elseif (auth()->user() && auth()->user()->avatar_url) {
                                $photoUrl = auth()->user()->avatar_url;
                            }
                        @endphp

                        @if($photoUrl)
                            <img src="{{ $photoUrl }}" alt="{{ auth()->user()->name }}" class="w-full h-full rounded-[14px] object-cover border border-white/20">
                        @else
                ...
                <div class="space-y-1.5">
                    <div class="flex flex-wrap items-center gap-2">
                        ...
                        <span class="px-3 py-1 rounded-full bg-white/10 border border-white/20 text-slate-200 text-xs font-mono font-bold">
                            {{ $registration?->registration_number ?? 'REG-2026-DZ' }}
                        </span>
                    </div>

                    <h1 class="text-2xl sm:text-3xl font-black tracking-tight text-white">
                        {{ auth()->user()->name ?? 'المتنافس الأولمبي' }}
                    </h1>
                    ...
                    <p class="text-xs sm:text-sm text-blue-100/90 font-bold flex items-center gap-2 pt-1">
                        <svg class="w-4 h-4 text-brand-sky" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19.428 15.428a2 2 0 00-1.022-.547l-2.387-.477a6 6 0 00-3.86.517l-.318.158a6 6 0 01-3.86.517L6.05 15.21a2 2 0 00-1.806.547M8 4h8l-1 1v5.172a2 2 0 00.586 1.414l5 5c1.26 1.26.367 3.414-1.415 3.414H4.828c-1.782 0-2.674-2.154-1.414-3.414l5-5A2 2 0 009 10.172V5L8 4z"/></svg>
                        <span>{{ app()->getLocale() === 'fr' ? 'Discipline: ' : (app()->getLocale() === 'en' ? 'Skill Trade: ' : 'التخصص التنافسي: ') }}</span>
                        <strong class="text-amber-300 font-extrabold">{{ $registration?->skill?->getLocalized('name') ?? 'التخصص الأولمبي الموحد' }}</strong>
                    </p>
                </div>
            </div>
            ...
            <div class="flex items-center gap-3 self-start md:self-auto">
                <button type="button" @click="showBadgeModal = true" class="px-5 py-3 rounded-2xl bg-gradient-to-r from-amber-400 to-amber-500 hover:from-amber-500 hover:to-amber-600 text-slate-950 font-black text-xs shadow-xl shadow-amber-500/20 transition transform hover:-translate-y-0.5 flex items-center gap-2">
    ...
    <!-- Official 5-Step Competition Milestone Journey -->
        ...
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-slate-100 pb-4">
            <div>
                <h3 class="text-base font-black text-slate-900 tracking-tight flex items-center gap-2">
                    ...
                    <span>{{ app()->getLocale() === 'fr' ? 'Parcours de Qualification et d\'Homologation' : (app()->getLocale() === 'en' ? 'Qualification & Accreditation Progress' : 'مسار التنافس والاعتماد الأولمبي الرسمّي') }}</span>
                </h3>
            ...
            <span class="px-3 py-1 rounded-full bg-brand-50 border border-brand-200 text-brand-700 text-xs font-black self-start sm:self-auto">
                {{ app()->getLocale() === 'fr' ? 'Étape 3 sur 5' : (app()->getLocale() === 'en' ? 'Step 3 of 5' : 'المرحلة 3 من أصل 5') }}
            </span>
        </div>

        <!-- Progress Track Bar -->
        <div class="relative">
            <div class="hidden sm:block absolute top-5 left-8 right-8 h-1 bg-slate-100 rounded-full">
                <div class="h-full bg-gradient-to-r from-emerald-500 via-emerald-400 to-brand-500 rounded-full transition-all duration-1000" style="width: 55%;"></div>
            </div>

            <div class="grid grid-cols-1 sm:grid-cols-5 gap-4 relative z-10">
                <!-- Step 1 -->
                <div class="p-4 rounded-2xl bg-emerald-50/80 border border-emerald-200/80 space-y-2 transition shadow-xs">
                    <div class="flex items-center justify-between">
                        <span class="w-7 h-7 rounded-xl bg-emerald-500 text-white font-black text-xs flex items-center justify-center">✓</span>
                        <span class="text-[10px] font-black text-emerald-700 uppercase">{{ app()->getLocale() === 'fr' ? 'Terminé' : (app()->getLocale() === 'en' ? 'Completed' : 'مكتمل') }}</span>
                    </div>
                    <h4 class="text-xs font-black text-emerald-950">{{ app()->getLocale() === 'fr' ? '① Soumission du dossier' : (app()->getLocale() === 'en' ? '① Application Submitted' : '① تقديم الطلب') }}</h4>
                    <p class="text-[11px] text-emerald-800 font-medium">{{ app()->getLocale() === 'fr' ? 'Votre dossier a été reçu et enregistré.' : (app()->getLocale() === 'en' ? 'Your file has been received and registered.' : 'تم استقبال ملفك وتسجيله بالبوابة.') }}</p>
                </div>
                ...
                <div class="p-4 rounded-2xl bg-emerald-50/80 border border-emerald-200/80 space-y-2 transition shadow-xs">
                    ...
                    <h4 class="text-xs font-black text-emerald-950">{{ app()->getLocale() === 'fr' ? '② Vérification Administrative' : (app()->getLocale() === 'en' ? '② Administrative Audit' : '② التدقيق الإداري') }}</h4>
                    <p class="text-[11px] text-emerald-800 font-medium">{{ app()->getLocale() === 'fr' ? 'Vérification de l\'âge et des certificats.' : (app()->getLocale() === 'en' ? 'Verification of age and requirements.' : 'التحقق الإداري من شروط السن والشهادات.') }}</p>
                </div>

                <!-- Step 3 (Active) -->
                <div class="p-4 rounded-2xl bg-gradient-to-br from-brand-50 via-blue-50 to-white border-2 border-brand-500 space-y-2 shadow-md relative overflow-hidden">
                    <div class="flex items-center justify-between">
                        <span class="w-7 h-7 rounded-xl bg-brand-600 text-white font-black text-xs flex items-center justify-center shadow-sm animate-bounce">3</span>
                        <span class="text-[10px] font-black text-brand-700 uppercase animate-pulse">{{ app()->getLocale() === 'fr' ? 'Étape Actuelle' : (app()->getLocale() === 'en' ? 'Current Step' : 'المرحلة الحالية') }}</span>
                    </div>
                    <h4 class="text-xs font-black text-brand-950">{{ app()->getLocale() === 'fr' ? '③ Homologation & Équipement' : (app()->getLocale() === 'en' ? '③ Accreditation & Readiness' : '③ الاعتماد والجاهزية') }}</h4>
                    <p class="text-[11px] text-brand-800 font-medium">{{ app()->getLocale() === 'fr' ? 'Fourniture du costume et des équipements.' : (app()->getLocale() === 'en' ? 'Provision of workwear and equipment sizes.' : 'توفير بدلة العمل وقياس التجهيزات اللوجستية.') }}</p>
                </div>

                <!-- Step 4 -->
                <div class="p-4 rounded-2xl bg-slate-50 border border-slate-200/60 space-y-2 opacity-75">
                    <div class="flex items-center justify-between">
                        <span class="w-7 h-7 rounded-xl bg-slate-200 text-slate-500 font-black text-xs flex items-center justify-center">4</span>
                        <span class="text-[10px] font-bold text-slate-400">{{ app()->getLocale() === 'fr' ? 'À venir' : (app()->getLocale() === 'en' ? 'Upcoming' : 'قادمة') }}</span>
                    </div>
                    <h4 class="text-xs font-black text-slate-700">{{ app()->getLocale() === 'fr' ? '④ Épreuves & Compétition' : (app()->getLocale() === 'en' ? '④ Field Competition' : '④ المنافسة الميدانية') }}</h4>
                    <p class="text-[11px] text-slate-500 font-medium">{{ app()->getLocale() === 'fr' ? 'Lancement des épreuves et évaluation du jury.' : (app()->getLocale() === 'en' ? 'Start of trials and jury evaluation.' : 'انطلاق التنافس وتدقيق لجان التحكيم.') }}</p>
                </div>

                <!-- Step 5 -->
                <div class="p-4 rounded-2xl bg-slate-50 border border-slate-200/60 space-y-2 opacity-75">
                    <div class="flex items-center justify-between">
                        <span class="w-7 h-7 rounded-xl bg-slate-200 text-slate-500 font-black text-xs flex items-center justify-center">5</span>
                        <span class="text-[10px] font-bold text-slate-400">{{ app()->getLocale() === 'fr' ? 'À venir' : (app()->getLocale() === 'en' ? 'Upcoming' : 'قادمة') }}</span>
                    </div>
                    <h4 class="text-xs font-black text-slate-700">{{ app()->getLocale() === 'fr' ? '⑤ Certification & Cérémonie' : (app()->getLocale() === 'en' ? '⑤ Certification & Award' : '⑤ الشهادة والتتويج') }}</h4>
                    <p class="text-[11px] text-slate-500 font-medium">{{ app()->getLocale() === 'fr' ? 'Annonce des résultats et remises des médailles.' : (app()->getLocale() === 'en' ? 'Results announcement and official certificates.' : 'إعلان النتائج وتوثيق الشهادات الوطنية.') }}</p>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        <div class="bg-white rounded-3xl p-6 shadow-xl border border-slate-200/80 space-y-4 hover:border-brand-300 transition group">
            <div class="flex items-center justify-between">
                ...
                <span class="px-2.5 py-1 rounded-lg bg-slate-100 text-slate-600 text-[10px] font-mono font-bold">
                    {{ $registration?->skill?->code ?? 'SKILL-2026' }}
                </span>
            </div>
            ...
            <div class="space-y-1">
                ...
                <h3 class="text-lg font-black text-slate-900 leading-snug">
                    {{ $registration?->skill?->getLocalized('name') ?? 'Cyber Security' }}
                </h3>
            </div>
        ...
        <div class="bg-white rounded-3xl p-6 shadow-xl border border-slate-200/80 space-y-4 hover:border-amber-300 transition group">
            <div class="flex items-center justify-between">
                ...
                <span class="px-2.5 py-1 rounded-lg bg-emerald-50 text-emerald-700 border border-emerald-200 text-[10px] font-bold">
                    {{ app()->getLocale() === 'fr' ? 'Prêt pour Livraison' : (app()->getLocale() === 'en' ? 'Ready for Delivery' : 'جاهز للتسليم') }}
                </span>
            </div>
            ...
            <div class="pt-3 border-t border-slate-100 grid grid-cols-2 gap-3">
                <div class="p-2.5 rounded-xl bg-slate-50 border border-slate-100 text-center">
                    ...
                    <strong class="text-sm font-black text-slate-900">{{ $profile?->suit_size ?? 'L' }}</strong>
                </div>
                <div class="p-2.5 rounded-xl bg-slate-50 border border-slate-100 text-center">
                    ...
                    <strong class="text-sm font-black text-slate-900">{{ $profile?->shoe_size ?? '42' }}</strong>
                </div>
            </div>
        </div>
            ...
            <div class="bg-gradient-to-b from-[#06205C] to-[#020A24] rounded-3xl p-6 text-white space-y-4 shadow-xl border border-white/20 relative">
                ...
                <div class="flex items-center justify-between">
                    ...
                    <span class="text-xs font-mono font-bold text-blue-200">
                        {{ $registration?->registration_number ?? 'WSAP-2026-DZ' }}
                    </span>
                </div>
                ...
                <div class="flex flex-col items-center space-y-2">
                    <div class="w-24 h-24 rounded-2xl border-2 border-amber-400 overflow-hidden shadow-md bg-slate-800">
                        @if($photoUrl)
                            <img src="{{ $photoUrl }}" alt="{{ auth()->user()->name }}" class="w-full h-full object-cover">
                        @else
                    ...
                    <h4 class="text-lg font-black text-white">{{ auth()->user()->name }}</h4>
                    <span class="text-xs text-amber-300 font-bold">{{ $registration?->skill?->getLocalized('name') ?? 'Cyber Security' }}</span>
                </div>
                ...
                <div class="bg-white p-3 rounded-2xl w-36 h-36 mx-auto flex items-center justify-center border-2 border-amber-400/50 shadow-inner">
                    <img src="{{ \App\Services\QrCodeService::generateDataUri(route('certificate', ['number' => $registration?->registration_number ?? 'WSAP-2026-DZ']), 150) }}" alt="QR Code" class="w-full h-full object-contain">
                </div>

                <p class="text-[10px] text-slate-300 font-mono">
                    Token: {{ substr($registration?->verification_token ?? 'WSAP-VERIFY-TOKEN', 0, 16) }}...
                </p>
            </div>

            <div class="flex items-center justify-center gap-3 pt-2">
                <a href="{{ route('official.certificate', ['identifier' => $registration?->registration_number ?? 'WSAP-2026-DZ']) }}" target="_blank" class="px-5 py-2.5 rounded-xl bg-[#0066FF] hover:bg-[#0052CC] text-white font-bold text-xs transition shadow-md flex items-center gap-2">
