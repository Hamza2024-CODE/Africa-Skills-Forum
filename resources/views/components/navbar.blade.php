<header class="sticky top-0 z-50 wsap-glass bg-white/95 border-b border-slate-200/80 shadow-sm" x-data="{ mobileMenuOpen: false }">
    <div class="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8">
        <div class="flex items-center justify-between h-16 sm:h-20 gap-2">
            ...
            @endphp
            <a href="{{ route('home') }}" class="flex items-center group shrink-0 py-1" title="WorldSkills">
                <img src="{{ $logoUrl }}" alt="WorldSkills Logo" class="h-10 sm:h-14 w-auto object-contain transition-transform duration-300 group-hover:scale-105">
            </a>
            ...
            <nav class="hidden lg:flex items-center gap-4 xl:gap-5">
                ...
                <div class="relative" x-data="{ open: false }">
                    <button @click="open = !open" @click.outside="open = false" class="flex items-center gap-1 text-xs font-bold text-[#06205C] hover:text-brand-500 transition py-2">
            ...
            <div class="flex items-center gap-1.5 sm:gap-2.5 shrink-0">
                ...
                @else
                    ...
                    <div class="relative shrink-0" x-data="{ regOpen: false }">
                        <button @click="regOpen = !regOpen" @click.outside="regOpen = false" type="button" class="px-3 sm:px-4 py-1.5 sm:py-2.5 rounded-xl bg-brand-500 hover:bg-brand-600 text-white font-bold text-xs shadow-md shadow-brand-500/20 transition-all flex items-center gap-1.5 whitespace-nowrap">
                        ...
                        <div x-show="regOpen" x-transition class="absolute top-full right-0 mt-2 w-72 rounded-2xl bg-white shadow-xl border border-slate-100 py-2 z-50 text-start">
                            <a href="{{ route('registration') }}" class="block px-4 py-2.5 text-xs font-bold text-[#06205C] hover:bg-slate-50 hover:text-brand-600 border-b border-slate-100">
                                <div class="font-extrabold text-slate-900 flex items-center gap-1.5 justify-start">
                            ...
                            <a href="{{ route('official.registration') }}" class="block px-4 py-2.5 text-xs font-bold text-[#06205C] hover:bg-slate-50 hover:text-brand-600">
                                <div class="font-extrabold text-indigo-900 flex items-center gap-1.5 justify-start">
                                    ...
                                    <span>{{ app()->getLocale() === 'fr' ? 'Inscription Officielle & Accréditation' : (app()->getLocale() === 'en' ? 'Official Registration & Accreditation' : 'التسجيل الرسمي والاعتماد') }}</span>
                                </div>
                                <div class="text-[10px] text-indigo-600 font-medium mt-0.5">
                                    {{ app()->getLocale() === 'fr' ? 'Jury, délégations, presse et médias' : (app()->getLocale() === 'en' ? 'Jury, delegations, press & media' : 'تسجيل الحكام والوفود والصحافة والإعلام') }}
                                </div>
                            </a>
                        </div>
                    </div>
                ...
                <button @click="mobileMenuOpen = !mobileMenuOpen; window.dispatchEvent(new CustomEvent('mobile-menu-toggled', { detail: mobileMenuOpen }))" type="button" class="lg:hidden w-8 h-8 sm:w-9 sm:h-9 rounded-xl bg-slate-100 hover:bg-slate-200 flex items-center justify-center text-[#06205C] transition shrink-0 ml-1">
    ...
    <template x-teleport="body">
        <div x-show="mobileMenuOpen" x-cloak class="fixed inset-0 z-50 lg:hidden flex justify-end">
            ...
            <div x-show="mobileMenuOpen"
                 ...
                 class="relative w-80 sm:w-96 max-w-[85vw] bg-white h-full shadow-2xl z-50 flex flex-col justify-between overflow-y-auto p-6 text-start border-s border-slate-200">
                ...
                <div class="space-y-6">
                    <div class="flex items-center justify-between border-b border-slate-100 pb-4">
                        @php
                            ...
                            $logoUrl = str_starts_with($siteLogo, 'http') ? $siteLogo : asset($siteLogo);
                        @endphp
                        <img src="{{ $logoUrl }}" alt="WorldSkills Logo" class="h-10 w-auto object-contain">
                        
                        <button @click="mobileMenuOpen = false; window.dispatchEvent(new CustomEvent('mobile-menu-toggled', { detail: false }))" type="button" class="w-9 h-9 rounded-full bg-slate-100 hover:bg-slate-200 text-slate-700 font-bold flex items-center justify-center transition">
                            ✕
                        </button>
                    </div>
                    ...
                    <nav class="space-y-1">
                        ...
                        <a href="{{ route('official.registration') }}" class="flex items-center gap-3 px-4 py-3 rounded-2xl text-xs font-black text-indigo-700 bg-indigo-50 hover:bg-indigo-100 border border-indigo-200 transition">
                            <svg class="w-4 h-4 text-indigo-600 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 6l3 1m0 0l-3 9a5 5 0 006.001 0M6 7l3 9M6 7l6-2m6 2l3-1m-3 1l-3 9a5 5 0 006.001 0M18 7l3 9m-3-9l-6-2m0-2v2m0 16V5m0 16H9m3 0h3"/></svg>
                            <span>{{ app()->getLocale() === 'fr' ? 'Inscription Officielle (Jury / Médias)' : (app()->getLocale() === 'en' ? 'Official Registration (Jury / Media)' : 'التسجيل الرسمي (حكام / وفود / صحافة)') }}</span>
                        </a>
