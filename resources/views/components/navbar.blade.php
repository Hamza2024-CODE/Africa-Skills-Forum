<header class="sticky top-0 z-50 wsap-glass bg-white/95 dark:bg-[#031826]/95 border-b border-slate-200/80 dark:border-[#094064] shadow-sm transition-colors duration-300" x-data="{ mobileMenuOpen: false }">
        ...
        <div class="flex items-center justify-between h-15 sm:h-20 gap-2 sm:gap-3">
            ...
            <nav class="hidden xl:flex items-center gap-3.5 2xl:gap-5 shrink">
                ...
                <div class="relative shrink-0" x-data="{ open: false }">
                    <button @click="open = !open" @click.outside="open = false" class="flex items-center gap-1 text-xs font-bold text-[#052D48] dark:text-slate-200 hover:text-[#24BDC3] dark:hover:text-[#24BDC3] transition py-2 whitespace-nowrap">
                    ...
                    <div x-show="open" x-transition class="absolute top-full right-0 mt-2 w-52 rounded-2xl bg-white dark:bg-[#052D48] shadow-xl border border-slate-100 dark:border-[#24BDC3]/30 py-2 z-50">
                        <a href="{{ route('news') }}" class="block px-4 py-2 text-xs font-bold text-[#052D48] dark:text-slate-200 hover:bg-slate-50 dark:hover:bg-[#083b5e] hover:text-[#24BDC3]">{{ __('messages.news') }}</a>
                        <a href="{{ route('events') }}" class="block px-4 py-2 text-xs font-bold text-[#052D48] dark:text-slate-200 hover:bg-slate-50 dark:hover:bg-[#083b5e] hover:text-[#24BDC3]">{{ __('messages.events') }}</a>
                ...
                @if(platform()->get('show_partners_section', true))
                    <a href="{{ route('partners') }}" class="text-xs font-bold text-[#052D48] dark:text-slate-200 hover:text-[#24BDC3] dark:hover:text-[#24BDC3] transition whitespace-nowrap">{{ __('messages.partners') }}</a>
                @endif
                <a href="{{ route('faq') }}" class="text-xs font-bold text-[#052D48] dark:text-slate-200 hover:text-[#24BDC3] dark:hover:text-[#24BDC3] transition whitespace-nowrap">{{ __('messages.faq') }}</a>
                <a href="{{ route('contact') }}" class="text-xs font-bold text-[#052D48] dark:text-slate-200 hover:text-[#24BDC3] dark:hover:text-[#24BDC3] transition whitespace-nowrap">{{ __('messages.contact') }}</a>
            </nav>
            ...
            <div class="flex items-center gap-1.5 sm:gap-2 shrink-0">
                ...
                <!-- Search Button -->
                <a href="{{ route('search') }}" class="w-7 h-7 sm:w-9 sm:h-9 rounded-full bg-slate-100 dark:bg-[#052D48] hover:bg-slate-200 dark:hover:bg-[#083b5e] flex items-center justify-center text-slate-600 dark:text-[#24BDC3] border border-transparent dark:border-[#24BDC3]/30 transition shrink-0" title="Search">
                ...
                <button @click="$store.theme.toggle()" 
                        type="button" 
                        class="w-7 h-7 sm:w-9 sm:h-9 rounded-full bg-slate-100 dark:bg-[#052D48] hover:bg-slate-200 dark:hover:bg-[#083b5e] flex items-center justify-center text-slate-700 dark:text-[#24BDC3] border border-slate-200 dark:border-[#24BDC3]/40 transition shrink-0 shadow-xs cursor-pointer"
                        :title="$store.theme.isDark ? 'Switch to Light Mode' : 'Switch to Dark Mode'"
                        aria-label="Toggle Theme Mode">
                    <svg x-show="$store.theme.isDark" class="w-4 h-4 text-[#24BDC3]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 3v1m0 16v1m9-9h-1M4 12H3m15.364 6.364l-.707-.707M6.343 6.343l-.707-.707m12.728 0l-.707.707M6.343 17.657l-.707.707M16 12a4 4 0 11-8 0 4 4 0 018 0z"/>
                    </svg>
                    <svg x-show="!$store.theme.isDark" class="w-4 h-4 text-[#052D48]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                @auth
                    ...
                    @endif
                @else
                    <a href="{{ route('login') }}" data-navigate-ignore class="hidden md:inline-flex items-center gap-1.5 px-3 sm:px-4 py-1.5 sm:py-2 rounded-xl border border-slate-300 hover:border-[#052D48] text-[#052D48] hover:bg-blue-50 font-bold text-xs transition whitespace-nowrap">
                ...
                <button @click="mobileMenuOpen = !mobileMenuOpen; window.dispatchEvent(new CustomEvent('mobile-menu-toggled', { detail: mobileMenuOpen }))" 
                        type="button" 
                        class="xl:hidden w-8 h-8 sm:w-9 sm:h-9 rounded-xl bg-[#052D48] hover:bg-[#031D2F] text-white flex items-center justify-center transition shrink-0 shadow-sm"
        ...
        <div x-show="mobileMenuOpen" x-cloak class="fixed inset-0 z-50 xl:hidden flex justify-end">
            ...
            <div x-show="mobileMenuOpen"
                ...
                <div class="space-y-5">
                    ...
                    <nav class="space-y-1 text-start">
                        ...
                        <a href="{{ route('news') }}" @click="mobileMenuOpen = false" class="flex items-center gap-3 px-3.5 py-2.5 rounded-xl text-xs font-black {{ request()->routeIs('news') ? 'bg-[#052D48] text-white shadow-md' : 'text-[#052D48] hover:bg-slate-50' }} transition">
                            <svg class="w-4 h-4 text-[#24BDC3]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 20H5a2 2 0 01-2-2V6a2 2 0 012-2h10a2 2 0 012 2v1m2 13a2 2 0 01-2-2V7m2 13a2 2 0 002-2V9a2 2 0 00-2-2h-2m-4-3H9M7 16h6M7 8h6v4H7V8z"/></svg>
                        ...
                        @if(platform()->get('show_partners_section', true))
                            <a href="{{ route('partners') }}" @click="mobileMenuOpen = false" class="flex items-center gap-3 px-3.5 py-2.5 rounded-xl text-xs font-black {{ request()->routeIs('partners') ? 'bg-[#052D48] text-white shadow-md' : 'text-[#052D48] hover:bg-slate-50' }} transition">
                                <svg class="w-4 h-4 text-[#24BDC3]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z"/></svg>
                                <span>{{ __('messages.partners') }}</span>
                            </a>
                        @endif

                        <a href="{{ route('faq') }}" @click="mobileMenuOpen = false" class="flex items-center gap-3 px-3.5 py-2.5 rounded-xl text-xs font-black {{ request()->routeIs('faq') ? 'bg-[#052D48] text-white shadow-md' : 'text-[#052D48] hover:bg-slate-50' }} transition">
                            <svg class="w-4 h-4 text-[#24BDC3]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8.228 9c.549-1.165 2.03-2 3.772-2 2.21 0 4 1.343 4 3 0 1.4-1.278 2.575-3.006 2.907-.542.104-.994.54-.994 1.093m0 3h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
                            <span>{{ __('messages.faq') }}</span>
                        </a>

                        <a href="{{ route('contact') }}" @click="mobileMenuOpen = false" class="flex items-center gap-3 px-3.5 py-2.5 rounded-xl text-xs font-black {{ request()->routeIs('contact') ? 'bg-[#052D48] text-white shadow-md' : 'text-[#052D48] hover:bg-slate-50' }} transition">
                            <svg class="w-4 h-4 text-[#24BDC3]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"/></svg>
                ...
                <div class="pt-5 border-t border-slate-100 space-y-2 mt-4">
                    @guest
                        <a href="{{ route('login') }}" @click="mobileMenuOpen = false" class="w-full py-2.5 rounded-xl border-2 border-[#052D48] text-center font-black text-xs text-[#052D48] hover:bg-slate-50 transition flex items-center justify-center gap-2">
