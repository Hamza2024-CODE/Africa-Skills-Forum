<div class="relative" x-data="{ open: false }">
    ...
    <div x-show="open" x-transition class="absolute top-full right-0 mt-2 w-32 rounded-2xl bg-white shadow-xl border border-slate-100 py-1.5 z-50 overflow-hidden">
        <a href="{{ route('lang.switch', 'ar') }}" @click.prevent="window.location.href = '{{ route('lang.switch', 'ar') }}'" data-navigate-ignore rel="external" class="flex items-center justify-between px-4 py-2 text-xs font-bold transition {{ app()->getLocale() === 'ar' ? 'text-[#0066FF] bg-blue-50 font-black' : 'text-[#06205C] hover:bg-slate-50' }}">
            <span>العربية</span>
            <span class="text-[10px] text-slate-400 font-mono">AR</span>
        </a>
        <a href="{{ route('lang.switch', 'fr') }}" @click.prevent="window.location.href = '{{ route('lang.switch', 'fr') }}'" data-navigate-ignore rel="external" class="flex items-center justify-between px-4 py-2 text-xs font-bold transition {{ app()->getLocale() === 'fr' ? 'text-[#0066FF] bg-blue-50 font-black' : 'text-[#06205C] hover:bg-slate-50' }}">
            <span>Français</span>
            <span class="text-[10px] text-slate-400 font-mono">FR</span>
        </a>
        <a href="{{ route('lang.switch', 'en') }}" @click.prevent="window.location.href = '{{ route('lang.switch', 'en') }}'" data-navigate-ignore rel="external" class="flex items-center justify-between px-4 py-2 text-xs font-bold transition {{ app()->getLocale() === 'en' ? 'text-[#0066FF] bg-blue-50 font-black' : 'text-[#06205C] hover:bg-slate-50' }}">
            <span>English</span>
